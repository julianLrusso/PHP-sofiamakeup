...
if(count($exDatos) === 0) {
    ...
    $exDatos = [
        'id_producto'	 => $producto->getProductoId(),
...
<section class="p-3">
<h2 class="my-3">Editar un producto del catálogo</h2>

<div class="row">
	<form class="col-md-6 mb-2" action="acciones/producto-modificar.php" enctype="multipart/form-data" method="post">
        <input type="hidden" name="id_producto" value="<?=$_GET['id'];?>">
        <input type="hidden" name="imagenActual" value="<?=$exDatos['imagenActual'] ;?>">
        ...
        <div class="form-group">
            <p>Imagen actual:</p>
            <img src="<?= RUTA_IMAGENES . $exDatos['imagenActual']?>" alt="<?= $exDatos['alt'] ?>">
        </div>
		<div class="form-group">
			<label for="imagen">Imagen</label> <span>(Solamente si se quiere cambiar la imagen actual. Sino, dejar vacío.)</span>
			...
			<select id="categoria" name="categoria" class="form-control">
				<?php foreach($categories as $cat): ?>
					<option value="<?=$cat->getCategoriaId();?>"
						<?= ($exDatos['categoria'] == $cat->getCategoriaId()) ? 'selected' : null;?>>
						<?=$cat->getNombre();?>
					</option>
				<?php endforeach ?>
			</select>
